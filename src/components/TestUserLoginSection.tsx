  ...
  const handleTestLogin = async () => {
    ...
    try {
      ...
      try {
        ...
        setError(locale === "pl" ? "Nie udało się zalogować. Spróbuj ponownie." : "Failed to sign in as test user. Please try again.");
      }
    } finally {
      setLoading(false);
    }
    ...
    <>
      <div className="rounded-xl border-2 border-dashed border-warning/30 bg-warning/5 p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {locale === "pl" ? "Zaloguj się od razu, żeby zobaczyć aplikację" : "Sign in instantly to explore the app"}
            </p>
          </div>
        </div>
        <Button
        ...
        >
          ...
          {loading ? t("auth.signingIn") : t("auth.testUser")}
        </Button>
        {error && (
      ...
      <div className="relative py-4">
        ...
        <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-background px-3 text-xs text-muted-foreground">
          {locale === "pl" ? "lub zaloguj się mailem" : "or continue with email"}
        </span>
      </div>
    </>
  );
